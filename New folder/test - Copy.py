...
def parse_bank_message(message):
    ...
    try:
        ...
        if re.search(r'pos purchase|شراء|عملية شراء', message_lower):
            transaction_type = 'مصروف'
        elif re.search(r'transfer|تحويل|حوالة', message_lower):
            transaction_type = 'تحويل'
        elif re.search(r'deposit|إيداع|رواتب|payroll', message_lower):
            transaction_type = 'دخل'
        elif re.search(r'online purchase|شراء اون لاين', message_lower):
            transaction_type = 'مصروف'
        
        # استخراج المبلغ
        amount_match = re.search(r'amount[:]?\s*([\d,.]+)\s*sar|مبلغ[:\s]*([\d,.]+)\s*ر\.س|بـ[:\s]*([\d,.]+)\s*sar', message_lower, re.IGNORECASE)
        amount = None
        if amount_match:
        ...
        # استخراج الجهة
        merchant = None
        merchant_match = re.search(r'at\s+(.+?)\s+by|عند\s+(.+?)\s+في|من[:\s]*([^\n]+)|لدى\s*([^\n]+)', message_lower, re.IGNORECASE)
        if merchant_match:
            for group in merchant_match.groups():
                if group and len(group.strip()) > 2:
                    merchant = group.strip()
                    break
        
        # استخراج التاريخ
        date_match = re.search(r'on[:]?\s*(\d{2}/\d{2}/\d{2,4})|في[:]?\s*(\d{2}/\d{2}/\d{2,4})|تاريخ[:\s]*(\d{2}/\d{2}/\d{4})', message_lower, re.IGNORECASE)
        date_str = None
        if date_match:
            for group in date_match.groups():
                if group:
                    date_str = group.strip()
                    break
        
        # 🔥 الجديد: التعرف على الحساب من خلال الأرقام
        account = None
        # البحث عن أرقام الحسابات في الرسالة
        for acc_number, acc_name in ACCOUNT_MAPPING.items():
            if acc_number in message:
                account = acc_name
                break
        
        # إذا لم يتم العثور على رقم حساب، استخدام الطريقة القديمة
        if not account:
            if re.search(r'credit card|بطاقة|ماجسترو|visa|mastercard', message_lower):
                account = '💳 ماستر'  # افتراضي للبطاقات
            elif re.search(r'account|حساب|بنك|bank', message_lower):
                account = '🏦 أهلي 136'  # افتراضي للحسابات
        ...
        print(f"Error parsing bank message: {e}")
        return None
# تنسيق البيانات للموافقة
def format_transaction_for_approval(transaction_data):
    """تنسيق بيانات المعاملة للموافقة عليها"""
    message = "✅ **تم التعرف على المعاملة:**\n\n"
    message += f"📋 **النوع:** {transaction_data['type']}\n"
    message += f"💰 **المبلغ:** {transaction_data['amount']:,.2f} ريال\n"
    
    if transaction_data['merchant']:
        message += f"🏪 **الجهة:** {transaction_data['merchant']}\n"
    
    if transaction_data['category']:
        message += f"🏷️ **التصنيف:** {transaction_data['category']}\n"
    
    if transaction_data['account']:
        message += f"🏦 **الحساب:** {transaction_data['account']}\n"
    
    if transaction_data['date']:
        message += f"📅 **التاريخ:** {transaction_data['date']}\n"
...
def execute_transfer(update, from_acc, to_acc, amount, accounts, transactions, transfers):
    ...
    update.message.reply_text(
        ...
        f"💵 الرصيد الجديد في:\n"
        f" {from_acc}: {from_balance} ريال\n"
        f" {to_acc}: {to_balance} ريال"
    )
    return ConversationHandler.END
