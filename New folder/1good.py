...
def handle_account_statement(update: Update, context: CallbackContext):
    try:
        ...
        if account_input in ['↩️ رجوع للقائمة الرئيسية', '❌ إلغاء']:
        ...
        account_name = get_account_name(account_input, accounts)
        if not account_name:
            update.message.reply_text("❌ الحساب غير موجود! حاول بإسم آخر:")
            return CATEGORY
...
def handle_action_selection(update: Update, context: CallbackContext):
    ...
    # إذا كان الأمر من الأوامر الرئيسية، الخروج من المحادثة والعودة للقائمة
    if action in ['📊 عرض الحسابات', '📈 عرض المصروفات', '🏦 إضافة حساب جديد', '📋 كشف حساب', '🔄 Start', '/start']:
        return start(update, context)
    ...
    if action == '↩️ رجوع للقائمة الرئيسية':
        ...
        # إعادة عرض الخيارات مع نفس البيانات
        keyboard = [
            ['💸 تسجيل كمصروف', '💰 تسجيل كدخل'],
            ['🔄 تسجيل كتحويل', '❌ إلغاء'],
...
def handle_message(update: Update, context: CallbackContext):
    ...
    if text == '📊 عرض الحسابات':
        ...
        # إذا لم يكن أمراً معروفاً، تجاهل بدلاً من محاولة استخلاص مبلغ
...
def main():
    try:
        ...
        # معالج المحادثة لكشف الحساب فقط
        account_statement_handler = ConversationHandler(
            entry_points=[MessageHandler(Filters.regex('^📋 كشف حساب$'), account_statement)],
            states={
        ...
        # معالجات منفصلة للعمليات الأخرى
        add_expense_handler = ConversationHandler(
            entry_points=[MessageHandler(Filters.regex('^➕ إضافة مصروف$'), add_expense)],
            states={ADD_EXPENSE: [MessageHandler(Filters.text & ~Filters.command, handle_add_expense)]},
            fallbacks=[CommandHandler('cancel', cancel)]
        )
        
        add_income_handler = ConversationHandler(
            entry_points=[MessageHandler(Filters.regex('^💸 إضافة دخل$'), add_income)],
            states={ADD_INCOME: [MessageHandler(Filters.text & ~Filters.command, handle_add_income)]},
            fallbacks=[CommandHandler('cancel', cancel)]
        )
        
        transfer_handler = ConversationHandler(
            entry_points=[MessageHandler(Filters.regex('^🔄 تحويل بين الحسابات$'), transfer_money)],
            states={TRANSFER: [MessageHandler(Filters.text & ~Filters.command, handle_transfer)]},
            fallbacks=[CommandHandler('cancel', cancel)]
        )
        
        new_account_handler = ConversationHandler(
            entry_points=[MessageHandler(Filters.regex('^🏦 إضافة حساب جديد$'), add_new_account)],
            states={NEW_ACCOUNT: [MessageHandler(Filters.text & ~Filters.command, handle_new_account)]},
            fallbacks=[CommandHandler('cancel', cancel)]
        )
        
        # معالج لاستخلاص المبالغ من النص (يجب أن يكون الأقل أولوية)
        extraction_handler = MessageHandler(
            Filters.text & 
            ~Filters.command & 
            ~Filters.regex('^📊 عرض الحسابات$|^📈 عرض المصروفات$|^🔄 Start$') &
            ~Filters.regex('^➕ إضافة مصروف$|^💸 إضافة دخل$|^🔄 تحويل بين الحسابات$|^🏦 إضافة حساب جديد$|^📋 كشف حساب$'),
            extract_amount_from_text
        )
        
        # معالج للأوامر العادية (يجب أن يكون الأعلى أولوية)
        normal_handler = MessageHandler(
            Filters.regex('^📊 عرض الحسابات$|^📈 عرض المصروفات$|^🔄 Start$'), 
            handle_message
        )
        
        # إضافة المعالجات بالأولوية الصحيحة (الأكثر تحديداً أولاً)
        dispatcher.add_handler(CommandHandler("start", start))
        dispatcher.add_handler(normal_handler)  # الأوامر العادية أولاً
        dispatcher.add_handler(account_statement_handler)
        dispatcher.add_handler(add_expense_handler)
        dispatcher.add_handler(add_income_handler)
        dispatcher.add_handler(transfer_handler)
        dispatcher.add_handler(new_account_handler)
        dispatcher.add_handler(extraction_handler)  # استخلاص المبالغ آخراً
