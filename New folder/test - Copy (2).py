...
def parse_bank_message(message):
    ...
    try:
        ...
        # 🔥 تحسين استخراج التاريخ - دعم صيغ أكثر
        date_str = None
        date_patterns = [
            r'(\d{1,2}/\d{1,2}/\d{2,4})',  # 12/31/2023
            r'(\d{1,2}-\d{1,2}-\d{2,4})',  # 12-31-2023
            r'(\d{1,2}\s*[-/]\s*\d{1,2}\s*[-/]\s*\d{2,4})',  # 12/31/23
            r'في[:]?\s*(\d{1,2}/\d{1,2}/\d{2,4})',  # في 12/31/2023
            r'تاريخ[:\s]*(\d{1,2}/\d{1,2}/\d{4})',  # تاريخ: 12/31/2023
            r'on[:]?\s*(\d{1,2}/\d{1,2}/\d{2,4})',  # on 12/31/2023
        ]
        ...
        for pattern in date_patterns:
            ...
            if date_match:
                ...
                # تحويل التاريخ إلى صيغة قياسية (YYYY-MM-DD)
                try:
                    if '/' in date_str:
                    ...
                    if len(parts) == 3:
                        ...
                        if len(year) == 2:  # إذا كانت السنة مكونة من رقمين
                            year = '20' + year  # افترض أنها في القرن 21
                        date_str = f"{year}-{month.zfill(2)}-{day.zfill(2)}"
                except:
                    # إذا فشل التحويل، استخدم التاريخ كما هو
                    pass
                break
        ...
        if transaction_type and amount:
            return {
                ...
                'amount': amount,
                'merchant': merchant,  # سيتم استخدامه كوصف
                'date': date_str,  # التاريخ المستخلص من الرسالة
                'account': account,
                'category': category
            }
