...
def start(update: Update, context: CallbackContext):
    keyboard = [
        ...
        ['📈 عرض المصروفات', '🏦 إضافة حساب جديد'],  # أضف فاصلة هنا,
...
def handle_message(update: Update, context: CallbackContext):
    ...
    if text == '📊 عرض الحسابات':
        ...
        add_new_account(update, context)  # أضف هذا السطر
    else:
...
def handle_account_statement(update: Update, context: CallbackContext):
    try:
        ...
        message = f"📊 **كشف حساب: {account_name}**\n\n"
        message += "💳 **المعاملات:**\n"
        ...
        if account_transactions.empty:
            ...
            for _, transaction in account_transactions.iterrows():
                emoji = "↗️ مصروف" if transaction['النوع'] == 'مصروف' else "↙️ دخل"
                message += f"{emoji}: {transaction['المبلغ']} ريال - {transaction['التصنيف']} ({transaction['التاريخ']})\n"
        
        message += "\n🔄 **التحويلات الصادرة:**\n"
        if outgoing_transfers.empty:
            message += "لا توجد تحويلات صادرة\n"
        else:
            for _, transfer in outgoing_transfers.iterrows():
                message += f"➡️ {transfer['المبلغ']} ريال إلى {transfer['إلى حساب']} ({transfer['التاريخ']})\n"
        
        message += "\n🔄 **التحويلات الواردة:**\n"
        if incoming_transfers.empty:
            message += "لا توجد تحويلات واردة\n"
        else:
            for _, transfer in incoming_transfers.iterrows():
                message += f"⬅️ {transfer['المبلغ']} ريال من {transfer['من حساب']} ({transfer['التاريخ']})\n"
        
        # حساب الرصيد الحالي
        current_balance = accounts[accounts['اسم الحساب'] == account_name]['الرصيد'].values[0]
        message += f"\n💰 **الرصيد الحالي: {current_balance:,.0f} ريال**"
...
def main():
    ...
    entry_points=[
        ...
        MessageHandler(Filters.regex('^📋 كشف حساب$'), account_statement)  # المدخل الجديد
    ],
    states={
        ...
        CATEGORY: [MessageHandler(Filters.text & ~Filters.command, handle_account_statement)]  # الحالة الجديدة
    },
